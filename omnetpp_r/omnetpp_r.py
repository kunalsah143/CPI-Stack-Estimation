...
train_df = pd.read_csv(io.BytesIO(uploaded['omnetpp_r_train.csv']))
...
#invoking the python library SDDRegressor() model and subsequetly fitting the train data set.
...
print("intercept",model.intercept_[0])  # print the intercpet value
...
test_df = pd.read_csv(io.BytesIO(uploaded_2['omnetpp_r_test.csv']))
...
CPI.ravel()
print("CPI(test)",np.average(CPI))	# avg CPI
...
Y_valid = res.iloc[:,-1]  # extract the target variable 
X_valid = res.iloc[:,0:10] # extract the feature variables 
