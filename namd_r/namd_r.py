...
train_df = pd.read_csv(io.BytesIO(uploaded['namd_r_train.csv']))
...
print("intercept",model.intercept_[0]	# printing the intercept value
...
test_df = pd.read_csv(io.BytesIO(uploaded_2['namd_r_test.csv']))
...
CPI.ravel()
print("CPI(test)",np.average(CPI))  # print the avg CPI
...
Y_valid = res.iloc[:,-1]	# extracting the target variable
X_valid = res.iloc[:,0:10]	# extrating the featire variables 
