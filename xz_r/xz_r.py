...
train_df = pd.read_csv(io.BytesIO(uploaded['xz_r_train.csv']))

from sklearn import linear_model,preprocessing

train_df.shape  # getting the dimensions of the train dataset.
train_df['CPI'] = train_df["cycles"]/train_df["instructions"]  #creating a new column 'CPI' which contains clocks per instruction value for each row.
...
i = 0
for feature_name in ["L1-dcache-load-misses","iTLB-load-misses","L1-icache-load-misses","LLC-store-misses","branch-misses:u","LLC-load-misses","dTLB-load-misses","dTLB-store-misses","L2-store-misses","L2-load-misses"]:
  print(feature_name,(model.coef_)[i])
  i = i + 1

print("intercept",model.intercept_[0])	#print the intercept value 
...
test_df = pd.read_csv(io.BytesIO(uploaded_2['xz_r_test.csv']))
...
Y_valid = res.iloc[:,-1]  # extracting the target column
X_valid = res.iloc[:,0:10] # extracting the feature columns 
